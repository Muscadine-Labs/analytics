...
import { formatCompactUSD, formatPercentage, formatAddress } from '@/lib/format/number';
...
export interface MarketRiskCardProps {
      ...
      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-2">
        <div className="flex-1">
          <div className="flex flex-wrap items-center gap-2">
            {marketKey ? (
              <a
                href={`https://app.morpho.org/base/market/${marketKey}`}
                target="_blank"
            ...
            <ScorePillar
              ...
              detail={
                oracleDisplay.lastUpdated ? (
                  <>
                    Last updated: {oracleDisplay.lastUpdated}
                    {oracleDisplay.age ? ` (${oracleDisplay.age})` : ''}
                  </>
                ) : (
                  <>
                    Last updated: unavailable
                    {market.oracleAddress
                      ? ` · Oracle ${formatAddress(market.oracleAddress)}`
                      : ''}
                  </>
                )
              }
